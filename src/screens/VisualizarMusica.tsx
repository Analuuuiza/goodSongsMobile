...
function VisualizarMusica(): React.JSX.Element {
    ...
    const buscar = async (titulo: string) => {
        try {
            const response = await axios.post('http://10.137.11.222:8000/api/musica/pesquisarPorTitulo', { titulo });
            console.log('buscando as músicas');
    ...
    const Delete = async (id: number) => {
        axios.delete(`http://10.137.11.222:8000/api/musica/excluirMusica/${id}`).then(function (response) {
            if(response.status === 200) {
    ...
    const renderItem = ({ item }: { item: Musica }) => (
        <View style={styles.form}>
            <View style={styles.card}>
                <Image style={styles.imagem} source={require('../assets/image/musica.png')} />
                ...
                <TouchableOpacity onPress={() => navigation.navigate('update', { item })} style={styles.alignConfig}>
                    <Image style={styles.configEdit} source={require('../assets/image/edit.png')} />
                </TouchableOpacity>
                <TouchableOpacity onPress={() => Delete(item.id)} style={styles.alignEdit}>
                    <Image style={styles.configDelete} source={require('../assets/image/deletee.png')} />
                </TouchableOpacity>
            </View>
        </View>
    );
        ...
        <View style={styles.container}>
            <View style={styles.header}>
                <View style={styles.row}>
                    ...
                    <TextInput placeholder="Search Music" placeholderTextColor={'grey'} style={styles.inputSearch}
                    onChangeText={(text) => text && buscar(text)}></TextInput>
                </View>         
            </View>
            {musicas.length === 0 ? (
